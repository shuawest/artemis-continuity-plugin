...
Alternatively, both sites can be kept active and the staging delay can be used as long as the delay is longer than the delta between the initial message produce, and acknowledgement recieve on the remote site side. This time delta average and maximum is measured by the plugin (and will be able to be monitored). 
